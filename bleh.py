# Adjusting the code to dynamically handle varying numbers of dimensions in the score vectors
import pandas as pd
import re
import matplotlib.pyplot as plt
from scipy.stats import f_oneway 
# Let's start by loading the three files to examine their structure and content.
file_paths = [
...
# Displaying the first few lines of each file for inspection
file_contents_preview = [content.split('\n')[:5] for content in file_contents]
file_contents_preview

# Function to dynamically parse the data from the file contents
...
# Creating a DataFrame with dynamic columns
...
# Handling missing values: we'll fill NaNs with 0 for simplicity
df_filled = df_dynamic.fillna(0)

# Separating the dataframe into two: one for good texts and one for bad texts
df_good = df_filled[df_filled['Text_Type'] == 'Good']
df_bad = df_filled[df_filled['Text_Type'] == 'Bad']

# Ensuring that the data is aligned properly
df_good = df_good.sort_values(by=['Prompt', 'Pair'])
df_bad = df_bad.sort_values(by=['Prompt', 'Pair'])

# Calculating the differences in scores between good and bad texts for each dimension
...
# Merging the good and bad dataframes on 'Prompt' and 'Pair' columns
...
# Keeping only the difference columns, along with 'Prompt' and 'Pair'
...
# Aggregate Analysis: Calculating mean, median, and standard deviation for each dimension and prompt
aggregate_stats = df_diff_corrected.groupby('Prompt')[diff_columns].agg(['mean', 'median', 'std'])

# Statistical Testing: ANOVA for each dimension across prompts
anova_results = {}
